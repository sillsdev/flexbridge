...
{
	[TestFixture]
	public class FieldWorksLintTypeHandlerTests : BaseFieldWorksTypeHandlerTests
	{
		...
		{
			FieldWorksTestServices.SetupTempFilesWithName(SharedConstants.LintFilename, out _ourFile, out _commonFile,
														  out _theirFile);
		}
		...
		public void ExtensionOfKnownFileTypesShouldBeTrans()
		{
			...
			Assert.IsTrue(extensions.Contains(SharedConstants.lint));
		}
		...
		public void ShouldNotBeAbleToValidateIncorrectFormatFile()
		{
			...
			{
				var newpath = Path.ChangeExtension(tempModelVersionFile.Path, SharedConstants.lint);
