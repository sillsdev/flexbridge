...
namespace FLEx_ChorusPlugin.Infrastructure
{
	...
	internal static class FieldWorksMergingServices
	{
		...
		{
			var srcObjsurNode = srcDoc.CreateNode(XmlNodeType.Element, FieldWorksMergeStrategyServices.Objsur, null);
			srcPropNode.AppendChild(srcObjsurNode);
			var srcGuidAttrNode = srcDoc.CreateAttribute(FieldWorksMergeStrategyServices.GuidStr);
				...
				else
				{
					...
					else
					{
						foreach (XmlNode ourOriginal in ourPropNode.ChildNodes)
							ourOriginalData.Add(XmlUtilities.GetStringAttribute(ourOriginal, "guid"), ourOriginal.OuterXml);
						ourPropNode.RemoveAll();
					}
					if (theirPropNode == null)
					{
					...
					else
					{
						foreach (XmlNode theirOriginal in theirPropNode.ChildNodes)
							theirOriginalData.Add(XmlUtilities.GetStringAttribute(theirOriginal, "guid"), theirOriginal.OuterXml);
						theirPropNode.RemoveAll();
					}
					foreach (var newValue in mergedCollection)
					{
		...
		private static void PreMerge(IMergeEventListener eventListener, MetadataCache mdc, XmlNode commonPropNode, XmlNode ourPropNode, XmlNode theirPropNode, HashSet<string> theirValues)
		{
			...
			{
				var ourChildGuid = XmlUtilities.GetStringAttribute(ourChild, FieldWorksMergeStrategyServices.GuidStr);
		...
		private static void PreMergeAtomicOwningProperties(IMergeEventListener eventListener, MetadataCache mdc, FdoClassInfo classInfo, XmlNode ourEntry, XmlNode theirEntry, XmlNode commonEntry)
		{
			...
			foreach (var ownAtomicProp in interestingProps)
			{
				...
				if (ourOwnedItem != null && ourOwnedItem.Name != FieldWorksMergeStrategyServices.Objsur
					&& theirOwnedItem != null && theirOwnedItem.Name != FieldWorksMergeStrategyServices.Objsur
					&& commonOwnedItem != null && commonOwnedItem.Name != FieldWorksMergeStrategyServices.Objsur
					&& ourOwnedItem.Attributes[FieldWorksMergeStrategyServices.GuidStr].Value == theirOwnedItem.Attributes[FieldWorksMergeStrategyServices.GuidStr].Value) // Allows for common to be different.
				{
