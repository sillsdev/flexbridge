...
namespace FLEx_ChorusPlugin.Infrastructure.Handling.General
{
	...
	/// </summary>
	internal class AnnotationFileTypeHandlerStrategy : IFieldWorksFileHandler
	{
		#region Implementation of IFieldWorksFileHandler

		public bool CanValidateFile(string pathToFile)
		{
			if (!FileUtils.CheckValidPathname(pathToFile, SharedConstants.Annotation))
				return false;
			if (Path.GetFileName(pathToFile) != SharedConstants.FLExAnnotationsFilename)
				return false;
		...
		private static void WritePreliminaryAnnotationsInformation(XmlReader reader, XmlWriter writer)
		{
			reader.MoveToContent();
			writer.WriteStartElement(SharedConstants.Annotations);
			reader.Read();
		}
	}
}