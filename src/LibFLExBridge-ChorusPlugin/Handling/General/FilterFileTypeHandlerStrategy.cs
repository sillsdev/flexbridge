	...
	internal sealed class FilterFileTypeHandlerStrategy : IFieldWorksFileHandler
	{
		...
		public string ValidateFile(string pathToFile)
		{
			try
			{
				...
				var root = doc.Root;
				if (root.Name.LocalName != FlexBridgeConstants.Filters
					|| root.Element(FlexBridgeConstants.Header) != null
					|| !root.Elements(FlexBridgeConstants.CmFilter).Any())
				{
					return "Not a valid filter file.";
				}

				return root.Elements(FlexBridgeConstants.CmFilter)
		...
		public IEnumerable<IChangeReport> Find2WayDifferences(FileInRevision parent, FileInRevision child, HgRepository repository)
		{
			return Xml2WayDiffService.ReportDifferences(repository, parent, child,
				null,
				FlexBridgeConstants.CmFilter, FlexBridgeConstants.GuidStr);
		}
		...
		{
			get { return FlexBridgeConstants.Filter; }
		}
