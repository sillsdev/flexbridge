...
namespace LibFLExBridgeChorusPlugin.Infrastructure.ActionHandlers
{
	...
	/// </summary>
	public class SendReceiveAction
	{
		/// <summary>
		/// Creates the chorus system.
		/// </summary>
		protected virtual ChorusSystemSimple CreateChorusSystem(string directoryName)
		{
			return new ChorusSystemSimple(directoryName);
		}

		protected virtual ChorusSystemSimple InitializeChorusSystem(string directoryName,
			string user)
		{
			var system = CreateChorusSystem(directoryName);
			system.Init(user);
			return system;
		}

		public SyncResults Run(string fullProjectPath, string user,
			Func<ChorusSystemSimple, SyncResults> syncFunc)
		{
			var projectDir = Path.GetDirectoryName(fullProjectPath);
			var projectName = Path.GetFileNameWithoutExtension(fullProjectPath);

			using (var chorusSystem = InitializeChorusSystem(projectDir, user))
			{
