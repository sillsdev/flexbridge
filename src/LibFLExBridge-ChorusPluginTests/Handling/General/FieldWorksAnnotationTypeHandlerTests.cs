...
namespace LibFLExBridgeChorusPluginTests.Handling.General
{
	...
	public class FieldWorksAnnotationTypeHandlerTests : BaseFieldWorksTypeHandlerTests
	{
		...
		{
			base.TestSetup();
			FieldWorksTestServices.SetupTempFilesWithName(FlexBridgeConstants.FLExAnnotationsFilename, out _ourFile, out _commonFile,
														  out _theirFile);
		}
		...
		public void ExtensionOfKnownFileTypesShouldBeTrans()
		{
			...
			Assert.IsTrue(extensions.Contains(FlexBridgeConstants.Annotation));
		}
		...
		public void ShouldNotBeAbleToValidateIncorrectFormatFile()
		{
			...
			{
				var newpath = Path.ChangeExtension(tempModelVersionFile.Path, FlexBridgeConstants.Annotation);
