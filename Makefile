...
release: vcs_version
	...
	cp -a run-in-environ environ environ-xulrunner run-flexbridge output/ReleaseMono
...
debug: vcs_version
	...
	cp -a run-in-environ environ environ-xulrunner run-flexbridge output/DebugMono
...
install:
	...
	/usr/bin/install environ environ-xulrunner run-in-environ run-flexbridge $(DESTDIR)/usr/lib/flexbridge
